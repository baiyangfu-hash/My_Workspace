...
def doc_sync_cmd(ctx: click.Context) -> None:
    ...
    console.print(
        Panel("[bold green]✨ Doc-as-Code 自动同步完成！[/bold green]", title="Doc Sync 结果")
    )
...
def doc_check_cmd(ctx: click.Context) -> None:
    ...
    if all_passed:
        console.print(
            Panel(
                "[bold green]🎉 所有文档一致性门禁检查 100% 通过！[/bold green]",
                title="Doc Check 结果",
            )
        )
    else:
        console.print(
            Panel(
                "[bold red]❌ 存在未同步的文档滞后项，请先执行 auto-pm doc sync 修复！[/bold red]",
                title="Doc Check 结果",
            )
        )
