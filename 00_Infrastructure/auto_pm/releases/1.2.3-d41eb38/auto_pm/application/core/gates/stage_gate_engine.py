...
class StageGateEngine:
    ...
    def _evaluate_g1_initiating_to_planning(self, p: Path) -> list[GateCheckItemDTO]:
        ...
        items.append(
            GateCheckItemDTO(
                ...
                severity="BLOCKER",
                message="PM_SESSION 文件已就绪"
                if has_pm_session
                else "缺少根目录 PM_SESSION_<ID>.md",
        ...
        items.append(
            GateCheckItemDTO(
                ...
                passed=has_nongoals,
                severity="BLOCKER",
                message="已定义项目定位与明确边界"
                if has_nongoals
                else "PM_SESSION 缺少 non_goals 边界定义",
    ...
    def _evaluate_g2_planning_to_executing(self, p: Path) -> list[GateCheckItemDTO]:
        ...
        has_int = (
            any(f.exists() for f in int_candidates)
            or any(p.glob("**/*INT*.md"))
            or any(p.glob("**/*IO*.md"))
        )
        items.append(
            GateCheckItemDTO(
                ...
                passed=has_int,
                severity="BLOCKER",
                message="接口/IO变量表契约已存在"
                if has_int
                else "未找到接口文档 (002_INT.md 或 IO分配表)",
        ...
        items.append(
            GateCheckItemDTO(
                ...
                message="已包含 HMI 原型或 DESIGN.md"
                if has_hmi
                else "未找到 HMI 交互原型 (.html 或 DESIGN.md)",
    ...
    def _evaluate_g3_executing_to_monitoring(self, p: Path) -> list[GateCheckItemDTO]:
        ...
        items.append(
            GateCheckItemDTO(
                ...
                severity="BLOCKER",
                message="无未决 P0 阻断性缺陷"
                if not has_blocker_defects
                else "存在未修复的 P0 阻断性缺陷",
        ...
        if closure_ok:
            ...
            if ledger_missing:
                closure_reasons.append(
                    f"台账缺失 {len(ledger_missing)} 条变更记录: {', '.join(ledger_missing[:3])}"
                )
            if not has_handoff:
    ...
    def _evaluate_g4_monitoring_to_closing(self, p: Path) -> list[GateCheckItemDTO]:
        ...
        for cf in chg_files:
            ...
            if (
                "status: closed" not in txt
                and "状态: closed" not in txt
                and "状态: 已关闭" not in txt
            ):
        ...
        items.append(
            GateCheckItemDTO(
                ...
                severity="BLOCKER",
                message="所有 CHG 变更单均已闭环"
                if all_chgs_closed
                else f"存在未闭环变更单: {', '.join(unclosed_chgs[:3])}",
