...
from auto_pm.ui.cli.decision import decision_group


def test_decision_package_dto_roundtrip():
...
def test_create_decision_rejects_unapproved_change(tmp_path: Path):
    """测试未批准的变更单被拒绝生成决策包"""
...
def test_create_decision_success_from_approved_change(tmp_path: Path):
    """测试已批准变更单成功生成并持久化决策包"""
    chg_dir = tmp_path / "01_变更单" / "CHG-SCPT"
    chg_dir.mkdir(parents=True)
    chg_file = chg_dir / "CHG-SCPT-2026-001.md"
