...
6. **自动化健康自检**（每次读取 PM_SESSION 后必须执行）：
   - 运行 `specmgr -w "<工作空间根>" check` 执行全量规范健康检查
   - 检查范围现已覆盖：
     - SHC-001~008：原有检查（重复/版本漂移/废弃引用/索引链接/未登记/前端链接/Frontmatter/规则路径）
     - 🆕 SHC-009：PM_SESSION 引用完整性（检查所有 PM_SESSION 中引用的文件路径是否存在）
     - 🆕 SHC-010：规范交叉引用完整性（检查各规范 .md 文件之间的交叉引用链接是否有效）
   - 将检查结果摘要报告给用户，标注新增/预存/已修复问题
   - 若发现可自动修复的问题，建议运行 `specmgr -w "<工作空间根>" check --auto-fix`
7. **代码规范合规自检**（当工作于 PLC 项目时自动触发）：
...
- **SpecMgr 规范健康检查**（当本次事件涉及规范文件变更时必须执行）：
  - 运行 `specmgr -w "<工作空间根>" check` 检查规范一致性，将结果报告给用户
