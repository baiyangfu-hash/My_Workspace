...
| 版本号 | 变更内容 | 变更人 | 变更日期 |
|--------|----------|--------|----------|
| V9.2.0 | 补充 FB_2001 外部设备通信心跳丢失报警代码 (2010) 与报警字位映射 (STD-860/820) | plc-electrical-engineer | 2026-09-04 |
...
#### 4.4.2 外部设备通信心跳丢失报警映射说明 (STD-860/820)

FB_2001 遵循汽车级首出诊断标准 (STD-860) 与设备交互规范 (STD-820)，定义外部设备通信心跳丢失专用报警常量与全局报警字位映射：

- **报警代码常量**：`ALM_CODE_HEARTBEAT_LOST : INT := 2010;`（外部设备通信心跳丢失报警代码，STD-860/820）
- **报警字位掩码**：`ALM_BIT_HEARTBEAT_LOST : WORD := 16#0010;`（全局报警字 `o_wGlobalAlarmWord` 第 4 位：外部设备通信心跳丢失）
- **映射逻辑与处理规则**：
  1. 当输送机 (`i_iConveyorAlarm`)、取放料 (`i_iPickPlaceAlarm`) 或打胶送料 (`i_iFeederAlarm`) 任一工站上报报警码为 2010 时，全局报警字自动通过位或置位 `16#0010`。
  2. 报警码 2010 参与最高优先级（数值最小）判定，若无更小报警码，则赋给 `o_iCurrentAlarmCode`。
  3. 支持上升沿触发新报警标志 `o_bNewAlarmFlag` 与 MES 报警队列去重记录。
