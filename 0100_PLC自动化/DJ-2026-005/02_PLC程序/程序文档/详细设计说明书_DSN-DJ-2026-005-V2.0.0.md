...
> 说明：本索引用于定位原始程序导出页面（LD/FBD/ST/FB）；梳理目标为模块/流程与关键互锁的语义对齐，不做地址等价迁移。
...
|--------|----------|--------|----------|----------|
| V2.0.0 | 交付口径统一到V2.0.0；补充编译器导出程序索引；同步文档引用 | Trae | 2026-05-17 | 文件名与文内版本号统一到V2.0.0；取消CC-Link强绑定表述（保留历史参考）；补充导出PNG索引便于溯源 |
| V4.2.0 | 全面规范化整理: 文件命名优化(参照DJ-2026-005), 文档策略调整, 编码标准化, 组件扩展(FB_ExternalDeviceInteraction) | Trae | 2026-04-29 | 源代码文件去除版本号后缀(6个); 功能块文档去版本号(~21个); 删除中文旧文件; external目录英文名标准化; UTF-8无BOM全项目覆盖; 新增FB_ExternalDeviceInteraction组件设计说明 |
...
graph TD
    ...
    subgraph 工站功能块
        Conveyor[FB_1001_Conveyor4Layer<br/>含4个FB_1002实例]
    ...
    subgraph 公共服务
        ...
        External[FB_3001_ExternalDeviceInteraction]
    end
...
> **源程序真相**：Step_S不连续编号(0→1→2→10→20→30→50→60→70)。先输送再分料。Step0初始化为输送反转复位。

单层输送机9步状态机:

| 步骤名称 | 步序号 | 动作描述 | 跳转条件 |
|---------|:-----:|---------|---------|
| STEP_初始化 | 0 | 输送反转复位到初始位 | 限位或超时→Step1 |
| STEP_等待来料 | 1 | 等待分料前感应器信号 | 分料前感应器=ON→Step2 |
| STEP_阻挡下降 | 2 | 阻挡气缸下降，阻止后续材料 | 阻挡下位=ON→Step10 |
| STEP_输送正转 | 10 | 输送带正向运转 | 延时/到位→Step20 |
| STEP_到位检测与阻挡上升 | 20 | 双传感器检测+阻挡上升释放 | 到位1&到位2=ON, 阻挡上位=ON→Step30 |
| STEP_分料推出 | 30 | 分料气缸推出分离边框 | 分料下位=ON→Step50 |
| STEP_慢速送出 | 50 | 慢速精确送出到取料位 | 延时→Step60 |
| STEP_请求取料 | 60 | 等待取放料取走 | 取料确认→Step70 |
| STEP_分料复位 | 70 | 分料气缸复位 | 分料上位=ON→Step0 |
...
实现Z轴升降、X1轴横移和夹爪控制，包含6步完整取放料循环(STL S20~S25)，一次取两根边框。

#### 4.2.2 状态机设计

> **源程序真相**：使用三菱STL步进梯形图，S20~S25共6步。产品检测在S22步内完成，不独立成步。放料完成与松开在同一S25步。

| 步骤名称 | 步序号 | 动作描述 | 跳转条件 |
|---------|:-----:|---------|---------|
| 待机等待 | S20 | Z轴待机位确认，等待取料请求 | 取料请求→S21 |
| 上升到取料高度 | S21 | Z轴移动到取片教点(D514) | 到位→S22 |
| 夹紧与检测 | S22 | 4组夹紧气缸同时夹紧+4光电全检+夹紧确认时间 | 4夹紧确认+4光电全ON→S23 |
| 移动到放料位置 | S23 | X1轴按层选点(L1/L3→D520, L2/L4→D540) | 到位→S24 |
| 下降到放料高度 | S24 | Z轴移动到放片教点(D524) | 到位→S25 |
| 松开与通知 | S25 | 4组夹紧全松开+发送放料完成给送料机构 | 松开确认→S20 |

#### 4.2.3 相关文档

| 文档类型 | 文档名称 | 核心内容 |
|---------|:--------|:--------|
| **详细设计说明书 (DSN)** | [详细设计说明书_DSN-FB1003-PickPlace-V4.1.0.md](取放料机构/详细设计说明书_DSN-FB1003-PickPlace-V4.1.0.md) | 6步状态机(S20~S25)、4层循环逻辑、双夹爪控制、产品检测 |
...
实现X2轴横移和打胶机交互，包含4步送料循环(D760=0→1→2→3)，支持安全区管理。

#### 4.3.2 状态机设计

> **源程序真相**：D760寄存器驱动4步步序。收到放料完成直接去取料位，不提前等待打胶机允许。

| 步骤名称 | D760值 | 动作描述 | 跳转条件 |
|---------|:-----:|---------|---------|
| 空闲待机 | 0 | 等待取放料机构"放料完成" | 放料完成→D760=1 |
| 移动到取料位 | 1 | X2轴移动到取料点(D620) | 到位→D760=2 |
| 移动到打胶机送料 | 2 | X2→放料点(D630), 关Y47安全区, 等X76→发Y44, 等X102 | X102=ON→D760=3 |
| 返回待机位 | 3 | X2轴返回待机位(D610), 开Y47安全区 | 到位→D760=0 |

#### 4.3.3 相关文档

| 文档类型 | 文档名称 | 核心内容 |
|---------|:--------|:--------|
| **详细设计说明书 (DSN)** | [详细设计说明书_DSN-FB1004-GlueMachineFeeder-V4.1.0.md](打胶机送料机构/详细设计说明书_DSN-FB1004-GlueMachineFeeder-V4.1.0.md) | 4步状态机(D760)、安全区管理、打胶机交互时序 |
...
| 文档类型 | 描述 | 文件位置 |
|----------|------|----------|
| 程序架构文档 | 系统架构概览 | [程序架构文档_ARC-DJ-2026-005-V2.0.0.md](程序架构文档_ARC-DJ-2026-005-V2.0.0.md) |
| 程序设计总文档 | 整体程序设计 | [../程序文档/016_DJ-2026-005_PLC程序设计总文档_PLC-V2.0.0.md](../程序文档/016_DJ-2026-005_PLC程序设计总文档_PLC-V2.0.0.md) |
| IO分配表 | IO点位定义 | [../程序文档/015_DJ-2026-005_IO分配表_IO-V2.0.0.md](../程序文档/015_DJ-2026-005_IO分配表_IO-V2.0.0.md) |
