...
```st
TESTSUITE "FB_1004_GlueMachineFeeder_BufferFraming"
    DESCRIPTION "打胶机送料机构功能块测试"
    
    TESTCASE "初始化流程测试"
        DESCRIPTION "测试功能块初始化流程"
        
        VAR
            fb : FB_1004_GlueMachineFeeder_BufferFraming;
        END_VAR
        
        SET fb.i_b使能 := TRUE;
        SET fb.i_bX2轴_伺服故障 := FALSE;
        
        WAIT_CYCLES 2;
        
        ASSERT fb.o_b运行中 = FALSE;
        ASSERT fb.o_i当前状态 = 0;
        ASSERT fb.o_b故障 = FALSE;
    
    TESTCASE "伺服故障报警测试"
        DESCRIPTION "测试伺服故障检测"
        ...
        SET fb.i_bX2轴_伺服故障 := TRUE;
        WAIT_CYCLES 1;
        
        ASSERT fb.o_b故障 = TRUE;
        ASSERT fb.o_i本站报警代码 = 203;
END_TESTSUITE
```
