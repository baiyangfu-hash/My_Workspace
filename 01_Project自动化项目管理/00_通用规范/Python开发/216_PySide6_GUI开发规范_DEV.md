...
|------|------|---------|---------|---------|
| 2026-07-16 | V1.1.0 | 优化 | 新增结构化委托渲染与离线PDF导出规范 | fubai |
...
- 禁止在客户端中直接引入重量级 `QtWebEngine` (如 WebEngineView) 渲染文档（会导致打包体积暴增 150MB+ 且破坏内网离线稳定性）

## 10. 推荐高级开发模式（V1.1.0 新增）

### 10.1 结构化委托渲染 (Structural Delegate Rendering)
- **定义**：对于 Markdown、日志等复杂文本，应在 Python 后端使用解析器将其分词解析为结构化 JSON 块，传递给 QML 后使用 `ListView` + `Loader` 动态装配原生卡片组件进行渲染。
- **优点**：规避了富文本 HTML 在不同操作系统上的排版偏差，保证了 QML 磨砂玻璃、发光和微拟物卡片风格的 100% 还原，同时支持原生手势和局部重构。

### 10.2 零外部库依赖 PDF 离线导出
- **定义**：客户端导出 PDF 时，禁止依赖 `weasyprint`、`reportlab` 等外部大体积三方包，应直接使用 PySide6 内置的 `QTextDocument` 构建 HTML 文档，并使用 `QPrinter(QPrinter.OutputFormat.PdfFormat)` 执行本地静默打印输出。
- **优点**：保证了 100% 内网离线导出成功率，不会产生缺失系统字体或 GTK 环境依赖的闪退报错。
