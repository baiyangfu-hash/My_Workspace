...
import chg141_e2e_test
from chg141_e2e_test import __app_name__, __version__


def test_package_importable() -> None:
    """验证包可以被导入"""
    assert chg141_e2e_test is not None


def test_app_name() -> None:
    """验证 app_name 常量"""
    assert __app_name__ == "chg141-e2e-test"
...
def test_file_utils_roundtrip(tmp_workspace: str) -> None:
    """验证 file_utils 读写往返"""
    from chg141_e2e_test.utils.file_utils import read_file, write_file
...
def test_cli_hello_command() -> None:
    ...
    from chg141_e2e_test.cli.__main__ import cli

    runner = CliRunner()
    result = runner.invoke(cli, ["hello"])
    assert result.exit_code == 0
    assert "Hello from chg141_e2e_test" in result.output
...
def test_cli_help() -> None:
    ...
    assert result.exit_code == 0
    assert "chg141_e2e_test" in result.output
