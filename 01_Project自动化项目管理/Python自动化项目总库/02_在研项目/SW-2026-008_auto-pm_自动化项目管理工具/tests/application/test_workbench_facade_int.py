...
def test_workbench_facade_rebuild_index_and_stats(workbench_facade):
    ...
    assert "index.db" in payload.db_path


def test_workbench_facade_full_flow(workbench_facade):
    ...
    assert rebuild_res.payload.projects_found == 1

    # 2. 获取项目卡片列表（DB 缓存模式）
    cards_res = workbench_facade.list_project_cards()
    assert cards_res.success is True
    assert len(cards_res.payload) == 1
    card = cards_res.payload[0]
    ...
    assert ws_res.payload.project_id == "SW-2026-001"
    # asset_summary_service=None，所以 asset_summary 为 None
    assert ws_res.payload.asset_summary is None
...
def test_workbench_facade_clear_and_rebuild(workbench_facade):
    """清缓存后重建索引，验证数据一致性"""
    # 1. 先重建索引
    rebuild_res = workbench_facade.rebuild_index()
    assert rebuild_res.success is True
    ...
    assert clear_res.payload.success is True

    # 3. 再次重建索引，验证数据一致
    rebuild_res2 = workbench_facade.rebuild_index()
    assert rebuild_res2.success is True
    assert rebuild_res2.payload.projects_found == 1
