...
- current_focus: V2.3 Week2 已完成 + 全项目审查 + V2.3 Week3 转换器重建（2026-07-01 状态更新）。V2.3 Week2 T08-T11 多格式解析器全部完成（ProgramBlocksParser/CommunicationsParser/BaseParser+5格式Parser/format_detector + CLI 集成 --format/--output-format/list-formats/detect-format + 35 测试 + 全量回归 1409 passed 6 skipped 0 failed + ruff 0 + mypy auto_pm 0 errors/140 文件 + mypy tests 0 errors/128 文件 + DJ-2026-005 端到端 6 项验证通过）。V2.3 Week1 已收口（T01-T07 全部完成，1369 passed 6 skipped 0 failed，ruff/mypy auto_pm 0 errors）。用户决策推进顺序：① TD-C10 治理（mypy tests/ 381 errors 跨 45 文件，原留 V2.4 但用户要求提前治理）；② V2.3 Week2 多格式解析器实施；③ 全项目审查。前序 V0.4.2 整改批次已完成（22 项全部修复，CHG-079 closed）。V2.2 Week1-3 已全部完成（specmgr 吸收 + GUI 规范中心页改造 + TD-C07/C08/C09 清理 + CHG-078 第 9 次 dogfooding 闭环）；31/31 项技术债全部关闭；全量回归基线 1332 passed 5 skipped 0 failed；ruff 0 + mypy auto_pm 0 errors/126 文件。本轮基于审查报告 73 文件 62 项发现（9 Critical + 20 Major + 22 Minor + 11 Info），按用户决策执行 P0+P1 整改批次（22 项）：P0 9 项 Critical（DSN §9/§11/§10.2 修正 + 005_CHG-078 索引补登 + 元测试 AST 升级 + GUI 测试 tmp_path 隔离 DJ-2026-998 + CHG-077 表格修正 + 006 §0.1 总览表修正 + TD-TC01 状态统一）+ P1 13 项 Major（INT/DSN/TEC/REL 4 文档升级 V2.1.x + INT CLI/Service 补全 + 005_CHG V2.2 Week1-3 章节 + 008_PILOT CHG-078 第 9 次闭环 + 007_REL G3/§4.4 修正 + commands/check.py 死代码删除 + 4 处 except log + type:ignore 注释 + 测试基线对齐 1337 vs 1366 + CHG-077 模块路径 auto_am→auto_pm + 6 个 .log 删除 + .gitignore 追加 *.log/.auto-pm/ + tests/gui 8 处宽 except 评估 + TD-C10 登记 mypy tests/ 380 errors）。用户决策 2026-06-30：① 实施范围 P0+P1（22 项）；② 创建 CHG-SCPT-2026-079 走第 10 次 dogfooding 闭环（draft→closed 9 步状态流转，审查报告作为附件，P0+P1 作为变更内容）；③ pyproject 0.4.1→0.4.2 升级（CHANGELOG 新增 [0.4.2] 章节，PRD V2.1.2 对齐）；④ 切换到 fullstack-engineer 执行实施；⑤ 完成后由 pm-workflow 做 review，告知是否可以进入下个主线计划（V2.3 变量表解析整合）。
- milestone: 代码基线 0.4.2（pyproject.toml version=0.4.2；CHANGELOG [0.4.2] - 2026-06-30 已汇总 V2.2 Week1-3 + V0.4.2 整改批次；[Unreleased] 待 V2.3 全部收口后添加版本条目；全量回归基线 1409 passed 6 skipped 0 failed）/ PRD V2.1.2（V0.4.2 版本统一收口）
...
- [precondition: V2.3 Week4 已完成] [待启动] done_when: 全项目审查——用户要求 V2.3 Week4 后审查整个项目，覆盖代码质量/测试覆盖/文档一致性/技术债/版本号一致性/PM_SESSION 单一真源完整性
