...
- current_focus: V0.5.0 版本统一收口 + V2.3 Week1-4 已完成 + 文档对齐（2026-07-01 状态更新）。V2.3 Week1-4 变量表解析整合主线全部完成：Week1 数据模型+IoPointsParser+CLI vartable 命令组；Week2 多格式解析器+格式自动识别+35 测试；Week3 5 格式 Parser 深化+转换器重建+批量解析+35 测试；Week4 GUI 变量编辑器+项目工作区变量表 Tab+33 测试。32/32 项技术债全部关闭（TD-C10 mypy tests/ 381→0 errors 已在 V2.3 治理完成）。全量回归 1477 passed 6 skipped 0 failed。本轮文档整理：09_整改项归档 8 个历史文件；02_设计 GUI 原型升级到 V2.1（新增变量表 Tab 设计章节）；00_项目基础信息 8 文档对齐 V0.5.0/V2.2.0；006 技术债 §0.1 总览表修正到 32 项 + §12 长期治理建议。下一步进入 V0.5.x 稳定期 + V0.6.0 真实项目持续 dogfood + 长期路线规划阶段。
- milestone: 代码基线 0.5.0（pyproject.toml version=0.5.0；CHANGELOG [0.5.0] - 2026-07-01 汇总 V2.3 Week1-4 + TD-C10 治理完成；全量回归 1477 passed 6 skipped 0 failed；ruff 0 + mypy auto_pm 0 errors/140 文件 + mypy tests 0 errors/128 文件）/ PRD V2.2.0（V0.5.0 版本统一收口）
...
- current_state: 2026-07-01 V0.5.0 版本统一收口 + 文档整理已完成。本轮 pm-workflow 完成用户要求的三个目录文档整理（09_整改项 + 00_项目基础信息 + 02_设计），所有整理对齐 V0.5.0 基线（pyproject 0.5.0；CHANGELOG [0.5.0] - 2026-07-01；全量回归 1477 passed 6 skipped 0 failed）。关键交付：① 09_整改项归档整合（7 历史文件 → archive/，2 active 保留，README 索引建立）；② 00_项目基础信息 8 文档全部对齐 V0.5.0/V2.2.0（008_PILOT V1.8.0 含第 10 次闭环证据 + 006 技术债 §0.1 32/32 全关 + §12 长期治理建议 4 小节）；③ 02_设计 GUI 原型升级到 V2.1（§5.5 变量表 Tab 设计 9 小节对齐 V2.3 Week4 实现 + §14 V2.1 变更记录，**待用户审核**）；④ 里程碑迭代计划进度表从 0/27 更新到 26/27（96% 完成）+ 新增 V0.5.0+ 长期路线图（§8.1 V0.5.x 稳定期 Week 1-4 + §8.2 V0.6.0 长期路线 Week 5-8+ + §8.3 V0.7.0+ 更长期方向 + §8.4 长期治理原则 5 项）。32/32 项技术债全部关闭，剩余 0 项。CHG-SCPT-2026-080 状态保持 implementing（V2.3 Week1-4 已完成 T01-T16，待 V2.3 收尾后再流转到 closed）。dogfooding 累计 11 次闭环证据。**下一步：等待用户审核 GUI 原型 V2.1 升级；通过后进入 V0.5.x 稳定期 Week 1-4 治理重点（5 格式 Parser 真实样本覆盖 + VartableTab 角色权限后端打通 + VariableConverter 中间模型字段契约 + 测试执行耗时优化）**。
...
- [precondition: GUI 完整测试已完成 + 发现 TD-G01 P0 Bug + 12 个 P1 视觉问题] [待启动] done_when: 修复 GUI 测试发现的 3 个生产代码问题——① **TD-G01**（P0）：`auto_pm/ui/dialogs/new_project_dialog.py:317-324` 新建项目时 equipment_type/plc_vendor/plc_model 仅在 `if value` 时写入 data，未填时 copier 模板渲染为 null 导致 ProjectInfo Pydantic 校验失败，修复方案 A：提交时 `data[key] = value or ""` 或方案 B：ProjectInfo 模型字段改 `str = ""` 默认值；② **V-01/02/03**（P1）：`auto_pm/ui/workspace/overview_tab.py` 概览 Tab 内容溢出（控件(1098,1245)超出父(1098,676) 溢出 569px），修复：主内容容器包裹 `QScrollArea` + `setWidgetResizable(True)`；③ **V-04~V-12**（P1）：`auto_pm/ui/vartable/vartable_tab.py` QSplitter 子控件零宽度（9 个 zero_size 0x541/0x541/0x24），修复：Tab `showEvent` 调用 `splitter.adjustSize()` + 子控件 `setMinimumWidth(200)`；④ 修复后重新运行 `scripts/gui_plc_full_test.py` 回归验证 13 场景全部通过 + 0 Bug + 0 视觉问题；⑤ ruff 0 + mypy 0 + 全量回归 0 failed
- [precondition: TD-G01 + V-01~V-12 修复完成] [待启动] done_when: 升级 tests/gui/ 业务流程类 pytest 为"实际提交"测试——将 test_04/06/07/08/09/10/11 从"对话框能弹出+能填写"升级为"实际提交创建"（处理二级 QMessageBox 模态），与 scripts/gui_plc_full_test.py 形成"单元交互测试 + 端到端测试"分层覆盖；① test_04 实际新建项目（处理 QMessageBox 确认）；② test_06 实际创建变更单 + 实际状态流转（7 次）；③ test_07 实际执行 PLC 检查；④ test_09 变量表实际批量解析；⑤ test_10 变更中心实际创建变更单；⑥ 全部用 QTimer.singleShot 模式处理 exec() 阻塞；⑦ ruff 0 + mypy 0 + 全量回归 0 failed
