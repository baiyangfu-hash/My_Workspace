...
const ChangeModule = {
  ...
  _renderTransitionButtons(cr) {
    ...
    const FLOW = {
      ...
      implementing: [{ target: "pending_acceptance", label: "提交验收" }, { target: "approved", label: "退回已批准" }],
      pending_acceptance: [{ target: "accepting", label: "开始验收" }],
      accepting: [{ target: "completed", label: "完成验收" }, { target: "implementing", label: "退回返工" }],
