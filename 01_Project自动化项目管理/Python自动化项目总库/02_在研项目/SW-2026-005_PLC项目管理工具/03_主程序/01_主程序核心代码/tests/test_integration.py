...
class TestPLCServiceInitialization:
    """测试PLC服务的初始化和基础功能"""

    def test_01_plc_service_creation(self):
        """
        测试1: PLCService实例创建

        验证PLCService可以正常创建且初始状态正确。
        应该：
        - 成功创建实例
        - 检查器未初始化状态为False
        - 诊断器和服务实例为None
        ...
        assert service._checkers_initialized is False
        assert service._lsp_checker is None
        assert service._health_analyzer is None
        assert service._test_service is None

    def test_02_plc_service_initialize_checkers(self):
        """
        测试2: PLCService检查器初始化

        验证initialize_checkers()方法能成功注册所有内置检查器。
        应该：
        - 初始化返回True
        - 注册5个核心检查器（timer, naming, syntax, comment, config）
        - 重复调用不报错
        """
        from src.services.plc_service import PLCService
        ...
        # 第一次初始化
        result = service.initialize_checkers()
        assert result is True
        assert service._checkers_initialized is True
        assert len(service._checkers) >= 5  # 至少有5个核心检查器

        # 第二次初始化应该跳过
        result_second = service.initialize_checkers()
        assert result_second is True

    def test_03_plc_service_singleton_pattern(self):
        """
        测试3: PLCService单例模式

        验证get_plc_service()工厂函数返回相同的单例实例。
        应该：
        - 多次调用返回同一对象
        - 对象ID相同
        """
        from src.services.plc_service import get_plc_service

        instance1 = get_plc_service()
        instance2 = get_plc_service()

        assert instance1 is instance2
        assert id(instance1) == id(instance2)

    def test_04_plc_service_collect_st_files(self, temp_project_dir):
        """
        测试4: ST文件收集功能

        验证_collect_st_files能正确扫描并过滤ST源码文件。
        应该：
        - 找到.st扩展名文件
        - 排除测试文件（包含_test/test_）
        - 返回排序后的列表
        """
        from src.services.plc_service import PLCService
        ...
        # 创建额外的测试文件（应被排除）
        test_file = temp_project_dir / "test_example.st"
        test_file.write_text("(* test file *)")

        files = service._collect_st_files(str(temp_project_dir))
