# 架构设计文档 — V8.0.0 GUI数据流修复版

> **版本**: ARCH-V8.0.0
> **上一版本**: ARCH-V7.0.0 (全栈重构版)
> **最后更新**: 2026-06-03
> **变更摘要**: 修复 Store 状态管理 Proxy 桥接、IPC 事件推送、project.path 赋值、get_project_overview 数据格式、list_directory_tree 嵌套结构
...
│  │ • Template   │  │ • NumberGen  │  │ • WorkspaceGovern    │  │
│  │ • Diagnostic │  │ • TemplateRdr│  │ • Library            │  │
│  └──────┬───────┘  └──────┬───────┘  └──────────┬───────────┘  │
│         │                 │                      │              │
│  ┌──────┴─────────────────┴──────────────────────┴───────────┐ │
...
| 域 | Service | 公开接口 | 依赖 |
|----|---------|----------|------|
| **Core** | ProjectService | load_workspace, scan_project, get_project_detail, create_project | Repository |
| **Core** | DocumentService | create_document, list_documents, read_document, save_document | Repository, TemplateService |
| **Core** | TemplateService | get_templates, render_template | Repository |
| **Core** | DiagnosticService | run_diagnosis, get_cached_result | SpecCheckerService, Repository |
| **Change** | ChangeServiceV2 | create, transition, list, get, statistics, validate | Repository, ChangeNumberGenerator, ChangeTemplateRenderer |
| **Change** | ChangeNumberGenerator | generate, parse, list_existing | Repository |
| **Change** | ChangeTemplateRenderer | render, render_section | constants (枚举元数据) |
| **Workspace** | WorkspaceDashboardService | get_overview, get_project_cards, refresh | ProjectService, DiagnosticService, Repository |
| **Workspace** | WorkspaceService | mount, unmount, check, report | Repository |
| **Workspace** | LibraryService | get_library_summary, scan | Repository |
| **Infra** | SpecCheckerService | run_checks, get_checkers, run_single | checkers/*, Repository |
| **Infra** | AutoFixService | scan, preview, fix, rollback | fixers/*, Repository |
| **Infra** | SyncEngine | check_version, generate_chg, generate_ifc | Repository |
