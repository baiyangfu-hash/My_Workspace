...
### 修复
- **DevTools弹窗**: `main.py` 中 `debug=True` → `debug=False`，启动不再弹出Chromium开发者工具
- **版本号固化**: 创建 `config/app_config.json`，版本从硬编码 V1.0.0 → V4.3.0，后续修改配置文件即可更新
- **挂载Toast冲突**: 移除 `ipcMountWorkspace()` 中的重复成功Toast，统一由 `__ipc_recv('workspace_opened')` 显示，解决绿+红同时出现的问题
- **undefined标签页**: `renderTabBar()` 增加防御性过滤，自动清除无效(undefined/null)标签ID
- **工作空间图标**: 标签名 🌳 → 🌍 (更符合"工作空间"全球/多项目语义)

### 变更文件
| 文件 | 变更 |
|------|------|
| main.py | debug=False |
| src/core/config.py | version: "4.3.0" |
| config/app_config.json | **新增** - 持久化配置 |
| ui_prototype/js/ipc.js | 移除重复showToast |
| ui_prototype/js/navigation.js | tabNames图标更新 + renderTabBar防御过滤 |
| 版本变更台帐.md | 新增CHG-032记录 |
