...
result = engine.run_version_check()
# 返回: 版本差距矩阵 + 行动建议
```
...
```python
chg_path = engine.run_generate_chg(project_path)
# 返回: 生成的CHG文档路径
```
...
```python
ifc_path = engine.run_generate_ifc(project_path)
# 返回: 生成的IFC文档路径
```

---

## 9. Controller 层 API（V2.0-R2 新增）
...
controller.create_project(parent_widget)
controller.open_project(parent_widget)
controller.save_project()
controller.load_project_to_tree(project_path)
```
...
controller.run_version_check(project_path)
controller.run_generate_chg(project_path)
controller.run_generate_ifc(project_path)
```
...
## 10. 资源清理 API（V2.0-R2 新增）

### 10.1 MainWindow._cleanup_resources()

```python
# 窗口关闭前调用，清理所有后台资源
main_window._cleanup_resources()
# 遍历 DiagnosticPanel/SpecCheckPanel 调用各自的 cleanup()
```

### 10.2 Panel.cleanup()

```python
# DiagnosticPanel.cleanup() — 停止Worker线程 + 动画QTimer
# SpecCheckPanel.cleanup() — 停止CheckWorker线程
panel.cleanup()
```

---

## 11. EventBus API（V2.0-R2 更新）
...
```python
# 变更管理事件
sync_version_check_requested(str)    # 版本检查请求
sync_chg_generate_requested(str)     # CHG生成请求
sync_ifc_generate_requested(str)     # IFC生成请求
```

---

## 废弃 API 清单（V2.0-R2 删除）

| API | 原模块 | 删除原因 |
|-----|--------|----------|
| HMIService.* | hmi_service | 纯占位符 |
| PLCService.* | plc_service | 已废弃兼容层 |
| TestManagementService.* | test_management_service | 无UI接入点 |

---

*文档版本: INT-V2.0.0 | 最后更新: 2026-05-22*
