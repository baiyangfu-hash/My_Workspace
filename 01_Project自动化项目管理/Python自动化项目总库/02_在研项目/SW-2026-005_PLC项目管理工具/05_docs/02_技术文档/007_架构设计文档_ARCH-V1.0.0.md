...
- 现有工具在 V2.x 迭代中继续沿用当前架构，不新建平行工具。
- 新增项目类型 `DJ_SINGLE_MACHINE`，用于承载 DJ 单机项目模板和现有项目实例的导入治理。
- 项目资产扫描默认忽略 `.trae`、`.plc-out`、`.git`、`__pycache__`，避免将计划缓存和编译产物纳入治理范围。
- 文档治理遵循“文档先行、唯一权威源、不重复新增同类文档”的原则。
- 变更管理提升为一等核心模块，以 `00_项目管理/04_变更管理` 作为 Single Source of Truth。
...
**解决方案**: [PathResolver](file:///d:/BaiduSyncdisk/My_Workspace/01_Project自动化项目管理/Python自动化项目总库/02_在研项目/SW-2026-005_PLC项目管理工具/03_主程序/01_主程序核心代码/src/utils/path_resolver.py)
...
**解决方案**: [RuleRegistry](file:///d:/BaiduSyncdisk/My_Workspace/01_Project自动化项目管理/Python自动化项目总库/02_在研项目/SW-2026-005_PLC项目管理工具/03_主程序/01_主程序核心代码/src/checkers/rule_registry.py)
