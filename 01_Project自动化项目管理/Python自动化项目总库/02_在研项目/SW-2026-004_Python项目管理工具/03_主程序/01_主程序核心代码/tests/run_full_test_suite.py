...
class TestSuiteRunner:
    ...
    def __init__(self, mode='full', notify=False):
        ...
        self.report_dir = project_root / "data" / "test_reports"
        self._ensure_directories()
    
    def _ensure_directories(self):
        """确保必要目录存在"""
        os.makedirs(self.report_dir, exist_ok=True)
        os.makedirs(self.report_dir / 'realtime', exist_ok=True)
        os.makedirs(self.report_dir / 'detailed', exist_ok=True)
        os.makedirs(self.report_dir / 'trends', exist_ok=True)
    ...
    def _generate_detailed_report(self):
        ...
        report_file = self.report_dir / 'detailed' / f'详细测试报告_{timestamp}.md'
    ...
    def _generate_summary_report(self):
        """生成汇总测试报告"""
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        report_file = self.report_dir / f'测试报告_{timestamp}.md'
    ...
    def _generate_trend_report(self):
        ...
        # 生成趋势报告
        timestamp = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
        report_file = self.report_dir / 'trends' / f'趋势报告_{timestamp}.md'
    ...
    def _save_realtime_result(self, result):
        """保存实时结果"""
        realtime_file = self.report_dir / 'realtime' / 'current_test.json'
    ...
    def _load_trend_data(self):
        """加载趋势数据"""
        trend_file = self.report_dir / 'trends' / 'trend_data.json'
...
查看详细报告: data/test_reports/
        ...
        # 保存通知到文件
        notification_file = self.report_dir / 'notification.txt'
    ...
    def _log_event(self, level, message, data=None):
        """记录事件日志"""
        log_file = self.report_dir / 'test_execution.log'
        timestamp = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
