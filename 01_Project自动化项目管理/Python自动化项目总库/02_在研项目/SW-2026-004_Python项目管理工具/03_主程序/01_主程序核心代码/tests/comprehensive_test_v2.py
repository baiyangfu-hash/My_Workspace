...
class ComprehensiveTestRunner:
    ...
    def test_build_deploy(self):
        ...
        try:
            ...
            # 测试构建信息获取
            start_time = time.time()
            build_info = BuildDeployService.get_build_info(str(project_root))
            execution_time = time.time() - start_time
            self.result.add_test_case("构建部署", "获取构建信息", True, 
                                   f"项目类型: {build_info.get('构建类型', '未知')}", execution_time)
            
            # 测试项目路径验证
            start_time = time.time()
            exists = build_info.get("是否存在", False)
            execution_time = time.time() - start_time
            self.result.add_test_case("构建部署", "项目路径验证", True, 
                                   f"路径存在: {exists}", execution_time)
            
            # 测试Python项目检测
            start_time = time.time()
            is_python = build_info.get("是否为Python项目", False)
            execution_time = time.time() - start_time
            self.result.add_test_case("构建部署", "Python项目检测", True, 
                                   f"是Python项目: {is_python}", execution_time)
            
            # 测试构建功能（使用临时目录）
            import tempfile
            start_time = time.time()
            with tempfile.TemporaryDirectory() as temp_dir:
                success, error, result = BuildDeployService.build_project(
                    str(project_root),
                    output_dir=temp_dir
                )
                execution_time = time.time() - start_time
                if success:
                    self.result.add_test_case("构建部署", "项目构建测试", True, 
                                           f"构建结果: {result}", execution_time)
                else:
                    # 构建失败可能是环境问题，记录为警告而非失败
                    self.result.add_test_case("构建部署", "项目构建测试", False, 
                                           f"构建未成功: {error}", execution_time)
            
        except Exception as e:
            self.result.add_test_case("构建部署", "构建部署功能", False, str(e))
