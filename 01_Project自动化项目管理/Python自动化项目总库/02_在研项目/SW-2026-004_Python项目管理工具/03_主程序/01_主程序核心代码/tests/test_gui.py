...
class GUITestRunner:
    ...
    def test_main_window(self):
        ...
        def test_window_size():
            ...
            assert size.width() >= 1200, f"窗口宽度不足: {size.width()}"
            assert size.height() >= 800, f"窗口高度不足: {size.height()}"
        ...
        def test_status_bar():
            ...
            assert message == "就绪", f"状态栏初始消息不正确: {message}"
    ...
    def test_project_management(self):
        ...
        def test_search_input():
            ...
            project_list.search_input.setText("测试")
            QTest.qWait(100)
            assert project_list.search_input.text() == "测试", "搜索框输入失败"
        ...
        def test_filter_combos():
            ...
            assert project_list.status_filter.count() > 0, "状态筛选选项为空"
            assert project_list.business_filter.count() > 0, "业务线筛选选项为空"
        ...
        self._run_test(module, "刷新功能", test_refresh_function)
    ...
    def test_template_management(self):
        ...
        def test_load_templates():
            ...
            QTest.qWait(500)
            row_count = template_manager.table.rowCount()
            assert row_count >= 0, "模板加载失败"
        ...
        self._run_test(module, "筛选下拉框", test_filter_combos)
        self._run_test(module, "按钮存在性", test_buttons)
        self._run_test(module, "模板加载", test_load_templates)
    ...
    def test_plugin_management(self):
        ...
        def test_status_filter():
            ...
            assert plugin_manager.status_filter.count() > 0, "状态筛选选项为空"
        ...
        self._run_test(module, "按钮存在性", test_buttons)
        self._run_test(module, "插件加载", test_load_plugins)
    ...
    def test_spec_center(self):
        ...
        def test_category_combo():
            ...
            assert spec_center.category_combo.count() > 0, "分类选项为空"
        ...
        def test_refresh():
            ...
            QTest.qWait(500)
            assert spec_center.spec_tree.topLevelItemCount() >= 0, "规范树刷新失败"
    ...
    def test_change_management(self):
        ...
        def test_flow_buttons():
            assert change_manager.btn_submit is not None, "提交按钮不存在"
            assert change_manager.btn_approve is not None, "审批按钮不存在"
            assert change_manager.btn_reject is not None, "驳回按钮不存在"
        ...
        self._run_test(module, "流程按钮", test_flow_buttons)
    ...
    def test_progress_management(self):
        ...
        def test_progress_bar():
            ...
            assert progress_manager.progress_bar.minimum() == 0, "进度条最小值不正确"
            assert progress_manager.progress_bar.maximum() == 100, "进度条最大值不正确"
        ...
        def test_gantt_scroll():
            assert progress_manager.gantt_scroll is not None, "甘特图滚动区域不存在"
        ...
        self._run_test(module, "操作按钮", test_buttons)
        self._run_test(module, "甘特图区域", test_gantt_scroll)
    ...
    def test_report_center(self):
        ...
        def test_report_type_combo():
            ...
            assert report_center.report_type.count() > 0, "报告类型选项为空"
        ...
        def test_statistics_labels():
            assert report_center.total_projects_label is not None, "项目总数标签不存在"
            assert report_center.active_projects_label is not None, "进行中项目标签不存在"
        
        def test_quick_buttons():
            assert report_center.quick_stats_btn is not None, "快捷统计按钮不存在"
            assert report_center.quick_export_btn is not None, "快捷导出按钮不存在"
        ...
        self._run_test(module, "操作按钮", test_buttons)
        self._run_test(module, "统计标签", test_statistics_labels)
        self._run_test(module, "快捷按钮", test_quick_buttons)
    ...
    def test_plugin_config(self):
        ...
        def test_info_labels():
            ...
            assert plugin_config.author_label is not None, "作者标签不存在"
            assert plugin_config.status_label is not None, "状态标签不存在"
        ...
        def test_buttons():
            ...
            assert plugin_config.install_btn is not None, "安装按钮不存在"
            assert plugin_config.uninstall_btn is not None, "卸载按钮不存在"
        
        def test_config_tabs():
            assert plugin_config.config_tabs is not None, "配置标签页不存在"
            assert plugin_config.config_tabs.count() >= 2, "配置标签页数量不足"
