...
class LibraryManagerWidget(QWidget):
    ...
    def _load_libraries(self):
        ...
        try:
            ...
            logger.info(f"成功加载总库列表: {len(libraries)} 个总库")
        ...
        if ok and project_name:
            ...
            if project:
                ...
                if error:
                    ...
                    # 更新总库列表中的项目数量显示
                    self._refresh_current_library_count()
    ...
    def _refresh_current_library_count(self):
        """刷新当前选中总库在列表中的项目数量显示"""
        if not self.current_library_id:
            return
        ...
        try:
            ...
            for i in range(self.library_tree.topLevelItemCount()):
                item = self.library_tree.topLevelItem(i)
                item_library_id = item.data(0, Qt.UserRole)

                if item_library_id == self.current_library_id:
                    # 获取最新的项目数量
                    stats = LibraryService.get_library_statistics(self.current_library_id)
                    project_count = stats.get("project_count", 0)
                    # 更新显示
                    item.setText(2, str(project_count))
                    logger.debug(f"更新总库 {self.current_library_id} 的项目数量显示: {project_count}")
                    break
        except Exception as e:
            logger.exception(f"刷新总库项目数量失败: {e}")
