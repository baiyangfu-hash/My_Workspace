...
class TemplateManagerWidget(QWidget):
    ...
    def _load_templates(self):
        ...
        logger.info(f"========== 开始加载模板列表 ==========")
        logger.info(f"触发时间: {start_time.strftime('%Y-%m-%d %H:%M:%S.%f')}")

        # 【关键修复】刷新时重置所有筛选器到"全部"，避免残留的筛选条件导致只显示部分模板
        try:
            # 阻止信号以避免触发不必要的筛选操作
            self.business_line_filter.blockSignals(True)
            self.compiler_filter.blockSignals(True)
            self.scene_filter.blockSignals(True)
            self.search_input.blockSignals(True)

            # 重置业务线筛选器到"全部"
            self.business_line_filter.setCurrentIndex(0)
            logger.info(f"[筛选器重置] 业务线筛选器已重置到: {self.business_line_filter.currentText()}")

            # 重置编译器筛选器（将在 _update_filter_options() 中重新填充）
            self.compiler_filter.setCurrentIndex(0)
            logger.info(f"[筛选器重置] 编译器筛选器已重置到: {self.compiler_filter.currentText()}")

            # 重置场景筛选器（将在 _update_filter_options() 中重新填充）
            self.scene_filter.setCurrentIndex(0)
            logger.info(f"[筛选器重置] 场景筛选器已重置到: {self.scene_filter.currentText()}")

            # 清空搜索框
            self.search_input.clear()
            logger.info("[筛选器重置] 搜索框已清空")

            # 恢复信号
            self.business_line_filter.blockSignals(False)
            self.compiler_filter.blockSignals(False)
            self.scene_filter.blockSignals(False)
            self.search_input.blockSignals(False)

        except Exception as e:
            logger.error(f"重置筛选器时出错: {e}")
            # 确保信号被恢复
            try:
                self.business_line_filter.blockSignals(False)
                self.compiler_filter.blockSignals(False)
        ...
        try:
            ...
            duration = (load_time - start_time).total_seconds()
            logger.info(f"[数据库查询] 获取到 {len(self.templates)} 个模板 (耗时 {duration:.3f}s)")

            # 【防御性检查】如果模板数量异常少，记录警告
            if len(self.templates) == 0:
                logger.warning("⚠️ 模板列表为空！可能 initialize_builtin_templates() 未正确执行")
            elif len(self.templates) < 5:
                logger.warning(f"⚠️ 模板数量异常少: {len(self.templates)} 个 (预期 >= 5 个内置模板)")
                # 列出当前所有模板ID以便诊断
                template_ids = [t.template_id for t in self.templates]
                logger.warning(f"当前模板ID列表: {template_ids}")

            # 更新筛选选项（编译器和场景下拉框）
            self._update_filter_options()

            # 显示所有模板（此时筛选器已经重置，应该显示全部）
            self._display_templates(self.templates)

            end_time = datetime.datetime.now()
            total_duration = (end_time - start_time).total_seconds()
            logger.info(f"========== 模板加载完成 ==========")
            logger.info(f"总耗时: {total_duration:.3f}s, 显示模板数: {len(self.templates)}")

        except Exception as e:
            end_time = datetime.datetime.now()
            duration = (end_time - start_time).total_seconds()
            logger.exception(f"✗✗✗ 加载模板失败 (耗时 {duration:.3f}s): {e}")
            import traceback
            traceback.print_exc()
    ...
    def _display_templates(self, templates):
        ...
        if templates is None:
            logger.error("✗ 传入的模板列表为 None！")
            return

        if not templates:
            logger.warning("⚠️ 模板列表为空！可能原因：")
            logger.warning("  1. 数据库中确实没有模板记录")
            logger.warning("  2. initialize_builtin_templates() 未执行或执行失败")
            logger.warning("  3. 筛选条件过于严格导致无匹配结果")
            self.table.setRowCount(0)
            self.stats_label.setText("共 0 个模板 (0 个内置 / 0 个自定义)")
            return

        template_count = len(templates)
        logger.info(f"---------- 开始显示模板列表 ----------")
        logger.info(f"传入模板数量: {template_count}")

        # 【防御性检查】如果数量异常少，记录详细警告
        if template_count < 5:
            logger.warning(f"⚠️ 模板数量异常少: {template_count} (预期 >= 5)")
            logger.warning("当前模板详情:")
            for idx, tmpl in enumerate(templates):
                logger.warning(f"  [{idx+1}] {tmpl.template_id} - {tmpl.name} (内置: {tmpl.is_builtin})")

        self.table.setRowCount(template_count)
        logger.info(f"表格行数已设置为: {template_count}")
        ...
        for row, tmpl in enumerate(templates):
            try:
                logger.debug(f"处理第 {row} 行: {tmpl.template_id} - {tmpl.name}")
                ...
                logger.error(f"显示第 {row} 行模板失败 ({tmpl.template_id}): {e}")
                import traceback
                traceback.print_exc()
        ...
        display_end = datetime.datetime.now()
        display_duration = (display_end - display_start).total_seconds()
        logger.info(f"---------- 模板显示完成 ----------")
        logger.info(f"显示耗时: {display_duration:.3f}s")
        logger.info(f"统计: 总计 {total} 个 (内置: {builtin_count}, 自定义: {custom_count})")
