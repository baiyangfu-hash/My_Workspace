...
class MainWindow(QMainWindow):
    ...
    def _init_menu(self):
        ...
        version_history_action = QAction("版本记录", self)
    ...
    def _on_version_history(self):
        ...
        # 构建版本记录内容
        content = f"""
        <h1>Python项目管理工具 版本记录</h1>
        
        <h2>当前版本</h2>
        <p><strong>版本号:</strong> {VERSION}</p>
        <p><strong>构建日期:</strong> {VERSION_INFO.get('build', 'N/A')}</p>
        <p><strong>状态:</strong> {VERSION_INFO.get('status', 'N/A')}</p>
        <p><strong>变更内容:</strong> {VERSION_INFO.get('changelog', 'N/A')}</p>
        
        <h2>历史版本</h2>
        """
        
        # 读取发布说明目录中的更新说明文件
        release_notes_dir = Path(__file__).parent.parent.parent.parent / "02_发布说明"
        if release_notes_dir.exists():
            for file in sorted(release_notes_dir.iterdir(), reverse=True):
                if file.name.endswith('.md') and ('更新说明' in file.name or 'V2.' in file.name):
                    try:
                        with open(file, 'r', encoding='utf-8') as f:
                            md_content = f.read()
                        # 简单处理Markdown格式
                        md_content = md_content.replace('#', '<h3>')
                        md_content = md_content.replace('##', '<h4>')
                        md_content = md_content.replace('###', '<h5>')
                        md_content = md_content.replace('**', '<strong>')
                        md_content = md_content.replace('*', '<em>')
                        md_content = md_content.replace('\n', '<br>')
                        content += f"<h3>{file.name}</h3>{md_content}<hr>"
                    except Exception as e:
                        content += f"<h3>{file.name}</h3><p>读取失败: {str(e)}</p><hr>"
        else:
            content += "<p>未找到发布说明目录</p>"
