        ...
        try:
                    ...
                    if lib and lib.root_path:
                        ...
                        if lib_path.exists() and lib_path.is_dir():
                            ...
                            try:
                                ...
                                path_source = "总库root_path"
                            except Exception as write_err:
                                logger.warning(f"总库root_path不可写: {lib.root_path}, 错误: {write_err}")
                                path_source = "总库root_path(不可写，将回退)"
                        else:
                            logger.warning(f"总库root_path不存在: {lib.root_path}")
                            path_source = "总库root_path(不存在，将回退)"
                except Exception as e:
            ...
            if not base_path:
                try:
                    ...
                    if detected:
                        base_path = detected
                        path_source = "自动检测路径(_detect_project_base_path)"
                        logger.info(f"使用自动检测的项目基础路径: {detected}")
            ...
            if not base_path:
                ...
                logger.warning("使用当前工作目录作为项目路径基础")
            ...
            if base_path:
                ...
                # 如果使用了回退路径，在状态栏显示提示（可选）
                if "回退" in path_source or "自动检测" in path_source:
                    logger.info(f"项目路径生成完成 - 来源: {path_source}, 完整路径: {full_path}")
        except Exception as e:
