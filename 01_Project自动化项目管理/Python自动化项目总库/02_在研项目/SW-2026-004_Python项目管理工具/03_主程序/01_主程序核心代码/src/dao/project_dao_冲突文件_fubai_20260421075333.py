...
class ProjectDAO:
    ...
    def create(project: Project) -> Project:
        ...
        with db.get_session() as session:
            ...
            logger.info(f"创建项目 {project.project_id} 后清除缓存: {cleared_count} 条")
            return project
    ...
    def update(project_id: str, data: dict) -> Optional[Project]:
        ...
        with db.get_session() as session:
            ...
            # 清除所有项目相关缓存（包括统计缓存）
            cleared_count = cache.clear(namespace="projects")
            logger.info(f"更新项目 {project_id} 后清除缓存: {cleared_count} 条")
            return project
    ...
    def delete(project_id: str, hard_delete: bool = False) -> bool:
        ...
        with db.get_session() as session:
            ...
            # 清除所有项目相关缓存（包括统计缓存）
            cleared_count = cache.clear(namespace="projects")
            logger.info(f"删除项目 {project_id} (hard={hard_delete}) 后清除缓存: {cleared_count} 条")
            return True
    ...
    @staticmethod
    def count_by_status(status: Optional[ProjectStatus] = None, force_refresh: bool = False) -> int:
        """
        统计指定状态的项目数量

        Args:
            status: 项目状态，None表示统计所有项目
            force_refresh: 是否强制刷新缓存，忽略已有缓存数据
        ...
        cache_key = f"count:{status_value}"

        # 如果不是强制刷新，先尝试从缓存获取
        if not force_refresh:
            cached = cache.get(cache_key, namespace="projects")
            if cached is not None:
                logger.debug(f"从缓存获取项目统计: key={cache_key}, count={cached}")
                return cached
            else:
                logger.debug(f"缓存未命中: key={cache_key}, 将查询数据库")
        else:
            logger.info(f"强制刷新项目统计: key={cache_key}")
        ...
        with db.get_session() as session:
            ...
            # 更新缓存（使用较短的TTL以确保数据及时性）
            cache.set(cache_key, count, ttl=30, namespace="projects")
            logger.info(f"更新项目统计缓存: key={cache_key}, count={count}, ttl=30s")
