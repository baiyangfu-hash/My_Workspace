...
class LibraryService:
    ...
    def _detect_project_base_path() -> str:
        ...
        # 策略0: 直接返回 {base_search}/Projects（最高优先级）
        projects_path = base_search / "Projects"
        if projects_path.exists() or base_search.is_dir():
            try:
                projects_path.mkdir(parents=True, exist_ok=True)
                logger.info(f"策略0生效: 使用 {base_search}/Projects 作为项目根路径: {projects_path.resolve()}")
                return str(projects_path.resolve())
            except Exception as e:
                logger.warning(f"策略0创建Projects目录失败: {e}，将尝试后续策略")
    ...
    @staticmethod
    def cleanup_absolute_root_paths():
        """
        启动时清理含绝对路径的 root_path

        检测并清空 libraries 表中包含绝对路径（含盘符 ':'）的 root_path 字段，
        以避免因路径变更导致的异常。
        """
        try:
            ...
            try:
                ...
                for lib in libraries:
                    if lib.root_path and ":" in lib.root_path:
                        logger.warning(f"检测到旧绝对路径root_path，自动清空: {lib.root_path}")
                        lib.root_path = ""
                        cleaned_count += 1

                if cleaned_count > 0:
                    session.commit()
                    logger.info(f"已清理 {cleaned_count} 个含绝对路径的 root_path")
            finally:
                session.close()

        except Exception as e:
            logger.error(f"清理root_path失败: {e}")
    ...
    def initialize_default_library() -> tuple[Optional[Library], str]:
        ...
        # 启动时清理含绝对路径的 root_path
        LibraryService.cleanup_absolute_root_paths()
        ...
        try:
            ...
            try:
                ...
                if existing_libraries:
                    ...
                    # ===== 分类补全逻辑 =====
                    # 即使总库已存在，也检查并补全缺失的预置分类
                    existing_categories = session.query(Category).filter(
                        Category.library_id == existing.library_id
                    ).all()

                    expected_category_names = [BUSINESS_LINE_DESC.get(bl, bl.value) for bl in BusinessLine]
                    existing_category_names = {c.name for c in existing_categories}

                    missing_categories = set(expected_category_names) - existing_category_names

                    if missing_categories:
                        logger.info(f"检测到 {len(missing_categories)} 个缺失分类，开始补全: {missing_categories}")
                        for business_line in BusinessLine:
                            category_name = BUSINESS_LINE_DESC.get(business_line, business_line.value)
                            if category_name in missing_categories:
                                ...
                                if category:
                                    logger.info(f"补全分类成功: {category_name}")
                                else:
                                    logger.warning(f"补全分类失败: {category_name}, 错误: {cat_error}")
                    else:
                        logger.debug(f"分类完整性检查通过，共 {len(existing_categories)} 个分类")
                    # ===== 分类补全逻辑结束 =====
