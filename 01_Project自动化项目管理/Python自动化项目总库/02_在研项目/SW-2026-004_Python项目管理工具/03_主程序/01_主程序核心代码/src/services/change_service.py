...
class ChangeService:
    ...
    def export_change(change_id: str, format: str = "markdown", 
        ...
        try:
            ...
            # 确定输出路径 (V2.1.0: 04_变更管理/01_变更单/CHG-{DOMAIN}/)
            if not output_path:
                domain_dir = change.domain.value if hasattr(change, 'domain') and change.domain else 'PLC'
...
**审核人**: {reviewer}"""
    ...
    def generate_ledger(project_id: str, output_path: str = None) -> tuple[Optional[str], str]:
        ...
        try:
            ...
            # 确定输出路径 (V2.1.0: 04_变更管理/{项目}_版本变更台帐.md)
            if not output_path:
                output_path = os.path.join(
                    project.path,
                    "00_项目管理",
    ...
    def _generate_ledger_content(project, changes: List[Change]) -> str:
        ...
        for i, change in enumerate(changes, 1):
            domain_dir = change.domain.value if hasattr(change, 'domain') else 'PLC'
            link = f"[→ {change.change_id}](./01_变更单/CHG-{domain_dir}/{change.change_id}.md)"
...
- 变更单位于 `./01_变更单/CHG-{{DOMAIN}}/` 目录下按领域分类存放
